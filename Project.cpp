...
#if defined __ITEMFILTER && defined(__CLIENT)
	LoadItemFilter();
#endif
...
#if defined __ITEMFILTER && defined(__CLIENT)
bool CProject::LoadItemFilter()
{
	CScript s;
	if (!s.Load("ItemFilter.inc"))
	{
		Error(__FUNCTION__ " Cannot load ItemFilter.inc");
		return false;
	}

	ItemFilter Filter;
	s.GetToken();
	//Info(__FUNCTION__ " FIRST: %s", s.Token);
	while (s.tok != FINISHED)
	{
		int nID = s.GetNumber();
		//Info(__FUNCTION__ " ID: %d", nID);
		s.GetToken(); // {
		int nBlock = 1;
		while (nBlock && s.tok != FINISHED)
		{
			s.GetToken();
			//Info(__FUNCTION__ " %s", s.Token);
			if (s.Token == "{")
				nBlock++;
			else if (s.Token == "}")
				nBlock--;
			else if (s.Token == "TYPE")
			{
				s.GetToken();	// {
				s.GetToken();
				if (*s.token != '}')
					Filter._Type.nNumber = atoi(s.Token);

				while (*s.token != '}')
				{
					//Info(__FUNCTION__ " TYPE: %d", Filter._Type.nNumber);
					Filter._Type.dwIK1 = s.GetNumber();
					//Info(__FUNCTION__ " TYPE2: %d", Filter._Type.dwIK1);

					//Insert left
					Filter.m_vType.push_back(Filter._Type);
					Filter._Type.nNumber = s.GetNumber();
				}
			}
			else if (s.Token == "SECTION")
			{
				s.GetToken();	// {
				s.GetToken();
				strcpy(Filter._Section.szText, s.Token);
				//Info(__FUNCTION__ " SECTION: Weaponry vs %s", s.Token);
				s.GetToken();
				strcpy(Filter._Section.szIcon, s.Token);
				//Info(__FUNCTION__ " SECTION2: itm_WeaSwoAncient.dds vs %s", s.Token);
				Filter._Section.bActive = s.GetNumber();
				//Info(__FUNCTION__ " SECTION3: %d", Filter._Section.bActive);
				s.GetToken();
			}
			else if (s.Token == "ITEMKIND")
			{
				s.GetToken(); // {
				s.GetToken();
				if (*s.token != '}')
					Filter._Kind.dwIK3 = atoi(s.Token);

				while (*s.token != '}')
				{
					
					//Info(__FUNCTION__ " ITEMKIND: %s", s.Token);
					s.GetToken();
					strcpy(Filter._Kind.szText, s.Token);
					//Info(__FUNCTION__ " ITEMKIND2: %s", s.Token);
					s.GetToken();
					strcpy(Filter._Kind.szIcon, s.Token);
					//Info(__FUNCTION__ " ITEMKIND3: %s", s.Token);
					Filter._Kind.nNumber = s.GetNumber();
					//Info(__FUNCTION__ " ITEMKIND4: %d", Filter._Kind.nNumber);

					Filter.m_vKind.push_back(Filter._Kind);

					Filter._Kind.dwIK3 = s.GetNumber();
				}
			}
			else if (s.Token == "ITEM")
			{
				s.GetToken(); // {
				s.GetToken();
				...
				while (*s.token != '}')
				{
					...
					//Info(__FUNCTION__ " ITEM: %d", dwItem);
					int nNumber = s.GetNumber();
					//Info(__FUNCTION__ " ITEM2: %d", nNumber);

					Filter.m_vItem.push_back(make_pair(dwItem, nNumber));

					dwItem = s.GetNumber();
				}
			}
			else if (s.Token == "MENU")
			{
				s.GetToken(); // {
				s.GetToken();
				...
				int nNumber;
				if (*s.token != '}')
					nNumber = atoi(s.Token);

				while (*s.token != '}')
				{
					CString szText;
					//Info(__FUNCTION__ " MENU: %d", nNumber);
					s.GetToken();
					szText = s.Token;
					//Info(__FUNCTION__ " MENU2: %s", s.Token);

					Filter.m_vMenu.push_back(make_pair(nNumber, szText));

					nNumber = s.GetNumber();
				}
			}
		}
		m_mapItemFilter.insert(map<int, ItemFilter>::value_type(nID, Filter)); //Insert
		//Info(__FUNCTION__ " Insert: %d", m_mapItemFilter.size());

		s.GetToken(); //ID

		Filter.m_vType.clear();
		Filter.m_vItem.clear();
		Filter.m_vKind.clear();
		Filter.m_vMenu.clear();
		//Info(__FUNCTION__ " Token: %s", s.Token);
	}
