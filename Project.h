...
struct ItemFilter_Type
{
	int nNumber;
	DWORD dwIK1;
};

struct ItemFilter_Section
{
	char szText[64];
	char szIcon[64];
	bool bActive; //Remove ?
...
struct ItemFilter
{
	ItemFilter_Type _Type;
	ItemFilter_Section _Section;
	ItemFilter_ItemKind _Kind;
	std::vector<ItemFilter_Type> m_vType;
	std::vector<ItemFilter_ItemKind> m_vKind;
	std::vector<pair<DWORD, int>> m_vItem;
	std::vector<pair<int, CString>> m_vMenu;
};
#endif
...
public:
	void	SaveItemFilter();
	map<int, ItemFilter> m_mapItemFilter;
#endif
};
